...
for qubit in $QUBIT_COUNTS; do
    for depth in $DEPTHS; do
        if ([ "$depth" -gt 50 ] && [ "$qubit" -lt 40 ]) || 
           ([ "$depth" -gt 60 ] && [ "$qubit" -lt 50 ]); then
            continue
        fi
        for seed in $SEEDS; do
            file="pauli_gadget_${qubit}_${depth}_${MIN_WEIGHT}_${MAX_WEIGHT}_${seed}"
            echo $file
            if [ -f $file ]; then
                echo "EXISTS $file"
            else
                timeout $TIMEOUT ../../target/release/examples/pauli_gadget_stabrank_depth $qubit $depth $MIN_WEIGHT $MAX_WEIGHT $seed
